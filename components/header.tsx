...
export default function Header() {
  ...
  const [showContactInMain, setShowContactInMain] = useState(false)
  const [mounted, setMounted] = useState(false)

  React.useEffect(() => {
    setMounted(true);
    const handleScroll = () => {
      setShowContactInMain(window.scrollY > 40)
    }
  ...
  return (
    <>
      {/* Top Info Bar */}
      <motion.div 
        className="bg-gradient-to-r from-teal-600 to-blue-600 text-white py-2 sm:py-3 text-xs sm:text-sm backdrop-blur-xl"
      ...
      {/* Main Header */}
      <motion.header 
        className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl shadow-lg sticky top-0 z-50 border-b border-white/20"
        initial={{ y: -100 }}
        animate={{ y: 0 }}
